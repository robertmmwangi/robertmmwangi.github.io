---
title: "Projects"
permalink: /projects/
layout: single
author_profile: true
---

## 🌐 The Websites I Built

Before cybersecurity, I built websites. Lots of them. Each one taught me something about systems, clients, and problem-solving. Here are a few that I am proud of.

---

### 1.0 Halane Consultancy and Training Center (Germany) 🇩🇪

**URL:** [hctcentre.org](https://hctcentre.org) 🔗

I worked with a client in Hamburg, Germany. They needed a corporate website for their leadership training business. I built a clean, professional site that showcases their services, hosts their blog, and connects them with clients across Europe.

<div style="background: #f0f7ff; padding: 15px; border-radius: 10px; margin: 15px 0;">
  <strong>🛠️ What I used:</strong> WordPress, SEO, responsive design<br>
  <strong>📚 What I learned:</strong> International clients have different expectations. Communication across time zones requires discipline. A good website builds trust before a human says a word.
</div>

---

### 2.0 Shee Speaks – Storytelling Platform 📖

**URL:** [sheespeaks.co.ke](https://sheespeaks.co.ke) 🔗

Lucy Wanjiru is a writer and branding professional. She wanted a space to share her stories. I built her a blog platform that feels personal and warm. Readers can find posts by category. The design works on phones and computers.

<div style="background: #f0f7ff; padding: 15px; border-radius: 10px; margin: 15px 0;">
  <strong>🛠️ What I used:</strong> WordPress, content strategy, UI/UX design<br>
  <strong>📚 What I learned:</strong> Content is king. But structure is queen. A beautiful design means nothing if readers cannot find what they need.
</div>

---

### 3.0 Corporate Aid Solutions 🏢

**URL:** [corporateaidsolutions.co.ke](https://corporateaidsolutions.co.ke) 🔗

A corporate consultancy needed a web presence. I built a site that represents their brand and services. It is clean, professional, and ready for growth.

<div style="background: #f0f7ff; padding: 15px; border-radius: 10px; margin: 15px 0;">
  <strong>🛠️ What I used:</strong> WordPress, CMS management, performance optimization
</div>

---

### 4.0 SIDEC CBO 🌍

**URL:** [sidec-ke.org](https://sidec-ke.org) 🔗

This one is special. It is for a community-based organization focused on social impact. The site is launching soon. I am proud to support their mission.

<div style="background: #f0f7ff; padding: 15px; border-radius: 10px; margin: 15px 0;">
  <strong>🚀 Status:</strong> Launching soon<br>
  <strong>🛠️ What I used:</strong> WordPress, custom design
</div>

---

## 🔐 The Cybersecurity Projects That Taught Me

These are not just assignments. These are the moments where theory became practice.

---

### 5.0 Web Application Security Testing 🕸️

**The problem:** How do attackers find weaknesses in web applications?

**My approach:** I studied the OWASP Top 10 for 2025. I learned about SQL injection. I used FFUF to fuzz directories and find hidden endpoints. I manually tested for vulnerabilities.

<div style="background: #f0f7ff; padding: 15px; border-radius: 10px; margin: 15px 0;">
  <strong>🛠️ Tools:</strong> FFUF, OWASP guidelines, manual testing<br>
  <strong>💡 The lesson:</strong> Input validation saves lives. Well, maybe not lives. But definitely data. One unfiltered input field can destroy a database.
</div>

---

### 6.0 Network Traffic Analysis with Wireshark 📡

**The problem:** What happens on a network when no one is watching?

**My approach:** I captured live traffic. I filtered for DNS queries. I looked for patterns. I learned what normal looks like so I could spot abnormal.

<div style="background: #f0f7ff; padding: 15px; border-radius: 10px; margin: 15px 0;">
  <strong>🛠️ Tools:</strong> Wireshark, TCP/IP knowledge<br>
  <strong>💡 The lesson:</strong> A SOC analyst without traffic analysis skills is like a security guard who never watches the cameras.
</div>

---

### 7.0 Vulnerability Assessment and Enumeration 🔍

**The problem:** How do attackers find doors into a system?

**My approach:** I used Metasploit to scan networks. I enumerated open ports and running services. I documented everything.

<div style="background: #f0f7ff; padding: 15px; border-radius: 10px; margin: 15px 0;">
  <strong>🛠️ Tools:</strong> Metasploit Framework, TryHackMe labs<br>
  <strong>💡 The lesson:</strong> You cannot defend what you do not know exists. Enumeration is reconnaissance. Reconnaissance is the first step of every attack. And every defense.
</div>

---

## 📊 The Numbers

<div style="display: flex; flex-wrap: wrap; gap: 20px; margin: 30px 0;">
  <div style="flex: 1; min-width: 150px; background: #e8f4f8; padding: 20px; border-radius: 10px; text-align: center;">
    <div style="font-size: 2.5em;">🌐</div>
    <div style="font-size: 1.8em; font-weight: bold; color: #00C2FF;">4+</div>
    <div>Live Websites</div>
  </div>
  <div style="flex: 1; min-width: 150px; background: #e8f4f8; padding: 20px; border-radius: 10px; text-align: center;">
    <div style="font-size: 2.5em;">🌍</div>
    <div style="font-size: 1.8em; font-weight: bold; color: #00C2FF;">2</div>
    <div>Countries</div>
  </div>
  <div style="flex: 1; min-width: 150px; background: #e8f4f8; padding: 20px; border-radius: 10px; text-align: center;">
    <div style="font-size: 2.5em;">🏭</div>
    <div style="font-size: 1.8em; font-weight: bold; color: #00C2FF;">4+</div>
    <div>Industries</div>
  </div>
  <div style="flex: 1; min-width: 150px; background: #e8f4f8; padding: 20px; border-radius: 10px; text-align: center;">
    <div style="font-size: 2.5em;">🔐</div>
    <div style="font-size: 1.8em; font-weight: bold; color: #00C2FF;">3+</div>
    <div>Cyber Labs</div>
  </div>
</div>

---

## 🤝 Want to See More?

I am always building something new. Connect with me on [LinkedIn](https://linkedin.com/in/robert-mbuthia) or check my [GitHub](https://github.com/robertmmwangi).

<div style="text-align: center; margin: 30px 0;">
  <a href="/" style="background: #0077b5; color: white; padding: 10px 25px; border-radius: 25px; text-decoration: none;">🏠 Back to Home</a>
</div>
